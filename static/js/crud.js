document.addEventListener('DOMContentLoaded', function() {
    ...
    function crearFilaUsuario(usuario, index) {
        ...
        row.className = 'border-b border-gray-200 hover:bg-purple-50 transition-all duration-300 opacity-0 translate-x-4';
            ...
            <td class="px-4 py-3 text-sm">
                <div class="flex space-x-2">
                    <button onclick="editarUsuario(${usuario.id})" 
                        class="bg-blue-500 text-white px-3 py-1 rounded hover:bg-blue-600 transition-all transform hover:scale-110 text-xs font-semibold">
                        Editar
                    </button>
                    <button onclick="confirmarEliminar(${usuario.id})" 
                        class="bg-red-500 text-white px-3 py-1 rounded hover:bg-red-600 transition-all transform hover:scale-110 text-xs font-semibold">
                        Eliminar
                    </button>
                </div>
            </td>
    ...
    usuarioForm.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            if (data.success) {
                mostrarNotificacion(
                    modoEdicion ? 'Usuario actualizado exitosamente' : 'Usuario creado exitosamente',
                    'success'
                );
                cerrarModal();
                await cargarUsuarios();
